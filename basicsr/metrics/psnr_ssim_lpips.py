...
def calculate_lpips(img1,
    ...
    try:
        ...
        raise ImportError('LPIPS is not installed. Please install it with: pip install lpips')
    ...
    if img1_tensor.max() <= 1:
        img1_tensor = img1_tensor * 2 - 1
        img2_tensor = img2_tensor * 2 - 1
    else:
        img1_tensor = img1_tensor / 127.5 - 1
        img2_tensor = img2_tensor / 127.5 - 1
    ...
    if crop_border != 0:
        img1_tensor = img1_tensor[:, :, crop_border:-crop_border, crop_border:-crop_border]
        img2_tensor = img2_tensor[:, :, crop_border:-crop_border, crop_border:-crop_border]

    if test_y_channel:
        # Convert to Y channel
        img1_tensor = to_y_channel_tensor(img1_tensor)
        img2_tensor = to_y_channel_tensor(img2_tensor)
...
def to_y_channel_tensor(img_tensor):
    """Convert RGB tensor to Y channel tensor.
    
    Args:
        img_tensor (torch.Tensor): RGB tensor with shape (B, C, H, W).
        
    Returns:
        torch.Tensor: Y channel tensor with shape (B, 1, H, W).
    """
    # Convert to Y channel using the same coefficients as in metric_util.py
    # Y = 0.299 * R + 0.587 * G + 0.114 * B
    y_coeffs = torch.tensor([0.299, 0.587, 0.114]).view(1, 3, 1, 1)
    y_tensor = torch.sum(img_tensor * y_coeffs, dim=1, keepdim=True)
    return y_tensor
