...
def paired_paths_from_lmdb(folders, keys):
    ...
    if not (input_folder.endswith(".lmdb") and gt_folder.endswith(".lmdb")):
        raise ValueError(
            f"{input_key} folder and {gt_key} folder should both in lmdb formats. But received {input_key}: {input_folder}; {gt_key}: {gt_folder}"
        )
...
def paired_DP_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert (
        len(folders) == 3
    ), f"The len of folders should be 3 with [inputL_folder, inputR_folder, gt_folder]. But got {len(folders)}"
    assert (
        len(keys) == 3
    ), f"The len of keys should be 2 with [inputL_key, inputR_key, gt_key]. But got {len(keys)}"
    ...
    gt_paths = list(scandir(gt_folder))
    assert (
        len(inputL_paths) == len(inputR_paths) == len(gt_paths)
    ), f"{inputL_key} and {inputR_key} and {gt_key} datasets have different number of images: {len(inputL_paths)}, {len(inputR_paths)}, {len(gt_paths)}."
    paths = []
    for idx in range(len(gt_paths)):
        gt_path = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_path))
