...
def get_dataset_opt(opt):
    if "test" in opt["datasets"]:
        ...
        raise KeyError("The option file must contain `datasets.test` or `datasets.val`.")
...
def main():
    parser = argparse.ArgumentParser(description="Test RPIFormer on a paired low-light dataset.")
    parser.add_argument("--opt", type=str, required=True, help="Path to an option YAML file.")
    parser.add_argument("--weights", type=str, required=True, help="Path to the model checkpoint.")
    parser.add_argument("--dataset", type=str, required=True, help="Dataset name used for output folder naming.")
    parser.add_argument("--result_dir", type=str, default="results", help="Directory for test results.")
    parser.add_argument("--output_dir", type=str, default="", help="Optional flat output directory for restored images.")
    parser.add_argument("--gpus", type=str, default="0", help="CUDA visible devices, e.g. `0` or `0,1`.")
    parser.add_argument("--GT_mean", action="store_true", help="Rectify output brightness using GT mean (not recommended).")
    parser.add_argument("--self_ensemble", action="store_true", help="Apply x8 self-ensemble inference.")
    ...
    dataloader = create_dataloader(
        dataset, dataset_opt, num_gpu=opt.get("num_gpu", 1), dist=False, sampler=None, seed=opt.get("manual_seed")
    )
    ...
    with torch.inference_mode():
        for data_batch in tqdm(dataloader, total=len(dataset)):
            ...
            if h < 3000 and w < 3000:
                restored = self_ensemble(input_tensor, model) if args.self_ensemble else model(input_tensor)
            else:
            ...
            ssim_scores.append(image_utils.calculate_ssim(img_as_ubyte(target), img_as_ubyte(restored)))
            lpips_scores.append(image_utils.calculate_lpips(img_as_ubyte(target), img_as_ubyte(restored)))

            save_path = (output_dir / f"{Path(inp_path).stem}.png") if output_dir else None
            if is_sid:
                type_id = Path(inp_path).parent.name
                save_path = result_dir / type_id / f"{Path(inp_path).stem}.png"
                image_utils.save_img(result_dir_input / type_id / f"{Path(inp_path).stem}.png", img_as_ubyte(input_save))
                image_utils.save_img(result_dir_gt / type_id / f"{Path(inp_path).stem}.png", img_as_ubyte(target))
